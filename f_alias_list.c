...
int handle_alias_args(char **commands, list_t **out_addrs)
{
	...
	{
		was_alias = 0;
		len = _strlen(commands[j]);

		/* Check that if user is trying to print an alias */
		for (curr = *alias_addrs; curr != NULL; curr = curr->next)
		{
			if (_strncmp(curr->str, commands[j], len) == 0 && (curr->str)[len] == '=')
		...
		if (is_set_alias(commands[j]))
			set_alias(commands[j]); /* Validate that the set format is fine */
		else /* Print errors */
		{
			status = -1;
